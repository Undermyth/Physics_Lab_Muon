...
def fix_peak(x, y, noise_threshold = 0.8, not_on_line = 1, sample_interval = 1e-7):
    ...
    a = get_curve_a()
    b = get_curve_b(x, y, noise_threshold = noise_threshold)
    def curve(x):
        return -np.exp(a * x + b)
    ...
    # if have point on the descending, then we will solve an equation.
        ...
        root = fsolve(equation, [x[np.argmin(y)] - dropping_time, x[np.argmin(y)]])
...
if __name__ == '__main__':
    ...
    y = np.loadtxt('data3/data412.txt')
